...
export default function TabsLayout() {
  ...
  return (
    ...
    //   }}
    screenOptions={({ route }) => ({
        // 1. Spread all default screen options from your central config.
        //    This will correctly apply `headerShown: false` if it's defined in your config file.
        ...(defaultScreenOptions as BottomTabNavigationOptions),

        // 2. Dynamically set the tabBarIcon for the current screen (route).
        tabBarIcon: ({ focused, color, size }) => {
          const screenSpecificConfig = tabsConfig.screens.find(
            (s) => s.name === route.name
          )
