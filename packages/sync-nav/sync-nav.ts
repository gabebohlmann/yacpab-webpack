...
/**
 * This function was part of your original script.
 * It's used by the original parseNavigationConfig.
 */
  ...
  return factory.createObjectLiteralExpression(
    [
      ...
      factory.createPropertyAssignment(
        ...
        factory.createStringLiteral(
          screenDetails.href ||
            `/${screenDetails.parentName}/${screenDetails.name}`
        )
      ),
      factory.createPropertyAssignment(
        "options",
 ...
 * It now correctly uses the globally defined findAppNavigationStructureDeclaration.
 */
async function parseNavigationConfig(filePath) {
  try {
    ...
    // Logic to extract screens recursively
    ...
    // Parse flags (isAutoSaveOn, isEditing, commandsToExecute)
          ...
          if (ts.isIdentifier(declaration.name)) {
            ...
            if (declaration.initializer) {
              if (varName === "isAutoSaveEnabled") {
                // Corrected to isAutoSaveEnabled based on your layout.tsx
                isAutoSaveOn =
                ...
                // ... (commandsToExecute parsing logic remains the same as your original)
    ...
    // Use the globally defined findAppNavigationStructureDeclaration
    ...
    if (
      ...
      // Fallback or different strategy if findAppNavigationStructureDeclaration doesn't work or for other structures.
      // For now, we assume findAppNavigationStructureDeclaration works as per your original script.
      // If `findTabsScreensArrayNodeFromDeclaration` was the intended entry for screen parsing, that would need to be used.
      // However, the recursive approach above is more general.
      // The old logic was:
      // const tabsScreensArrayNode = findTabsScreensArrayNodeFromDeclaration(appNavDeclaration);
      // if (tabsScreensArrayNode) { ... parse elements ... }
      // This old logic would only find screens in (tabs)
      console.warn(
...
function identifyChanges(currentConfigScreens, previousConfigScreens) {
  ...
  for (const [prevKey, prevScreen] of previousScreenMap) {
    if (!currentScreenMap.has(prevKey)) {
      // Potential rename or deletion
      for (const [currKey, currScreen] of currentScreenMap) {
        if (
          !previousScreenMap.has(currKey) && // It's a new key
...
function getScreenPaths(screenName, parent) {
  ...
  if (parent.name === "(drawer)") {
    // Screens directly under (drawer)
    expoParentPathSegments.push("(drawer)");
    webParentPathSegments.push("(drawer)");
  } else if (parent.name === "(tabs)") {
    // Screens under (drawer)/(tabs)
    expoParentPathSegments.push("(drawer)", "(tabs)");
    webParentPathSegments.push("(drawer)", "(tabs)");
  } else {
    // Fallback or error for unknown parent structure, adjust as needed
    console.warn(
      `Unknown parent structure for screen '${screenName}' with parent '${parent.name}'. Defaulting paths.`
    );
  ...
  return {
    ...
    webPageDir: path.join(webParentDir, screenName),
    webPagePath: path.join(webParentDir, screenName, "index.tsx"),
  };
}
...
import { ${componentName} } from '#features/${screenName}/screen';

export default function ${capitalizeFirstLetter(screenName)}Route() {
...
async function deleteScreenFiles(screenName, parent) {
  const { featurePath, expoFilePath, webPageDir } = getScreenPaths(
    screenName,
    parent
  );
  const deletedPaths = [];
...
async function renameScreenFiles(oldScreen, newScreen) {
  const {
    ...
    expoFilePath: oldExpoPath,
    webPageDir: oldWebDir,
  } = getScreenPaths(oldScreen.name, oldScreen.parent);
  const {
    featurePath: newFeaturePath,
    expoFilePath: newExpoPath,
    webPageDir: newWebDir,
  ...
  const transformResult = ts.transform(sourceFile, [
    (context) => {
      ...
      return (sf) => {
        ...
        // If no import changes, still need to return the transformed body with original imports
...
// --- Core Logic (processBatchOfChanges, etc. from your original script) ---
// This section remains largely the same in structure, but the functions it calls
// (parseNavigationConfig, file system ops, AST modification) are now updated.
  ...
  try {
    ...
    for (const { oldScreen, newScreen } of updatedScreens) {
      ...
      if (
        ...
        // if name or parent changes, treat as move/rename for files
        const renamedFilePaths = await renameScreenFiles(oldScreen, newScreen);
        renamedFilePaths.forEach((p) => allGeneratedOrModifiedFiles.add(p));
      }
      // Always regenerate content for updates
      const featurePath = await generateFeatureScreen(
        newScreen.name,
        newScreen.componentName,
        newScreen.title || newScreen.name,
        true,
      ...
      if (featurePath) allGeneratedOrModifiedFiles.add(featurePath);
      const expoPath = await generateExpoFile(
        newScreen.name,
        newScreen.componentName,
        newScreen.parent,
        true,
      ...
      if (expoPath) allGeneratedOrModifiedFiles.add(expoPath);
      const webPath = await generateWebFile(
        newScreen.name,
        newScreen.componentName,
        newScreen.parent,
        true,
      ...
      if (webPath) allGeneratedOrModifiedFiles.add(webPath);
...
async function getLayoutImports(sourceFile) {
  const imports = [];
  ...
  sourceFile.statements.forEach((statement) => {
    if (ts.isImportDeclaration(statement)) {
      ...
      if (ts.isStringLiteral(moduleSpecifier)) {
        ...
        // Updated to match #features/.../screen alias
        const match = importPath.match(
          /^#features\/([a-zA-Z0-9_.-]+)\/screen$/
        );
        if (match) {
          const screenName = match[1]; // Use group 1 for the screen name
          if (
  ...
  const scanAppDirRecursive = async (
    ...
    for (const item of items) {
      ...
      if (item.isDirectory()) {
        ...
        } else if (platformMap === screens.web) {
          // Web screens are directories with index.tsx
          if (await fs.pathExists(path.join(itemPath, "index.tsx"))) {
            if (!platformMap.has(currentParentKey))
              platformMap.set(currentParentKey, new Set());
            platformMap.get(currentParentKey).add(item.name);
          }
        }
      } else if (
        item.isFile() &&
        item.name.endsWith(".tsx") &&
        item.name !== "_layout.tsx" &&
        item.name !== "layout.tsx" &&
        platformMap === screens.expo
      ) {
        // Expo screens are .tsx files directly in their parent layout dir
        if (!platformMap.has(currentParentKey))
          platformMap.set(currentParentKey, new Set());
        platformMap.get(currentParentKey).add(item.name.replace(".tsx", ""));
      }
    }
  };
  // Start scanning from the top-level app directories that might contain layouts
...
async function validateProjectConsistency(
  ...
  // Debug logs from your previous test
  // console.log('Type of layoutSourceFile:', typeof layoutSourceFile);
  // console.log('layoutSourceFile is null:', layoutSourceFile === null);
  // console.log('layoutSourceFile is undefined:', layoutSourceFile === undefined);

  const importsFromFunc = await getLayoutImports(layoutSourceFile); // ***** MODIFIED: Added await *****
  // console.log('Value from getLayoutImports (after await):', importsFromFunc); // Changed log to reflect await
  // console.log('Is array from getLayoutImports (after await):', Array.isArray(importsFromFunc));

  const actualImports = importsFromFunc || [];
  // console.log('Value of actualImports:', actualImports);
  // console.log('Is actualImports an array:', Array.isArray(actualImports));

  const declaredScreenMap = new Map(
    declaredScreens.map((s) => [`${s.parent.name}-${s.name}`, s])
  );
  const {
  ...
  for (const screen of declaredScreens) {
    ...
    if (!webParentScreens || !webParentScreens.has(screen.name)) {
      proposedFixes.push({
        description: `Screen '${screen.name}' in Web parent '${screen.parent.name}': Missing Web page directory/file.`,
        action: () =>
    ...
    // This is line (approx) 903 where the error occurred in your log
  ...
  actualExpoScreensByParent.forEach((screenSet, parentName) => {
    screenSet.forEach((screenName) => {
      if (
        ...
        proposedFixes.push({
          description: `Orphaned Expo file: '${screenName}.tsx' in '${parentName}'.`,
          action: () =>
  ...
  actualWebScreensByParent.forEach((screenSet, parentName) => {
    screenSet.forEach((screenName) => {
      // screenName here is the directory name for web
      if (
        !declaredScreens.some(
          (s) => s.name === screenName && s.parent.name === parentName
        )
        ...
        proposedFixes.push({
          description: `Orphaned Web page: '${screenName}' in '${parentName}'.`,
          action: () =>
            deleteScreenFiles(screenName, {
              name: parentName,
  ...
  if (proposedFixes.length === 0) {
    console.log(
      "✅ Project consistency validation passed. No discrepancies found."
    );
    return { fixesApplied: false, astModified: false };
  }
  ...
  if (!isInteractive) {
    console.log(
      "Non-interactive mode. Skipping automatic fixes. Discrepancies listed above."
    );
    return { fixesApplied: false, astModified: false };
  }
...
// ... (This function remains the same as your provided original script)
async function onConfigFileChanged(changedPath) {
  ...
  ); // Ensure these are read correctly
  ...
  if (hasPendingCliCommands) {
    ...
    for (const cmd of commandsToExecute.add || []) {
      ...
      const parentType = parentName === "(tabs)" ? "tabs" : "drawer"; // Determine parentType based on name
      const componentName =
    ...
    if (
      ...
      await modifyLayoutFileWithAst({ clearCommands: true }); // Still clear them
      astModifiedByCommands = true; // Mark as modified to trigger re-parse if necessary
    }
  }
  ...
  try {
    ...
    if (astChangedByCli) {
      ...
      if (!currentParsedConfig) {
        console.error(
          "Failed to re-parse config after direct CLI AST modification."
        );
        return;
      }
    } else {
