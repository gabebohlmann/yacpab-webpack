  ...
  const optionsProperties = [
    factory.createPropertyAssignment(
      ...
      factory.createStringLiteral(
        screenDetails.title || capitalizeFirstLetter(cleanFeatureName)
      )
    ),
  ];
  ...
  if (screenDetails.parentType === "tabs") {
    optionsProperties.push(
      factory.createPropertyAssignment(
        "tabBarIconName",
        factory.createStringLiteral(
          screenDetails.icon || cleanFeatureName.toLowerCase()
        )
      )
    );
    ...
    optionsProperties.push(
      factory.createPropertyAssignment(
        "drawerLabel",
        factory.createStringLiteral(
          screenDetails.label || capitalizeFirstLetter(cleanFeatureName)
        )
      )
    );
  }

  const defaultHrefParentSegment =
    screenDetails.parentName === "Root"
      ? ""
      : getRouteSegmentName(screenDetails.parentName);
                    ...
                    if (ts.isPropertyAssignment(optProp) && optProp.name) {
                      ...
                      if (ts.isStringLiteral(optProp.initializer)) {
                        ...
                        if (optName === "tabBarLabel")
                          screen.label = optProp.initializer.text; // Assuming label can be for either
                      } else if (
                        optName === "showOn" &&
                        ts.isArrayLiteralExpression(optProp.initializer)
                      ) {
                    ...
                    if (
                      ...
                      prop.initializer.elements.forEach((elementNode) => {
                        if (ts.isObjectLiteralExpression(elementNode)) {
                          ...
                          if (commandArg.name) {
                            // Expects full name like "settings/index"
  ...
  for (const [key, currentScreen] of currentScreenMap) {
    if (previousScreenMap.has(key) && !processedAsRenameNewKeys.has(key)) {
      ...
      const relevantPropsChanged =
        ...
        JSON.stringify(currentScreen.showOn) !==
          JSON.stringify(previousScreen.showOn); // Check showOn
...
function getScreenPaths(configScreenName, parent) {
  // configScreenName is like "settings/index" or "(home)/index"
  ...
  if (!parent || !parent.name || !parent.type) {
    console.warn(
      `Parent info incomplete for screen '${configScreenName}'. File paths might be incorrect for Expo/Web.`
    );
    return {
      featurePath,
      expoScreenDir: null,
  ...
  if (parentRouteSegmentName === "(drawer)") {
    ...
    // For any other named parent group
...
async function generateExpoFile(
  ...
  const { expoFilePath, expoScreenDir } = getScreenPaths(
    configScreenName,
    parent
  );
  ...
  if (!expoFilePath || !expoScreenDir) {
    console.warn(`Could not determine Expo path for ${configScreenName}.`);
    return null;
  }
...
async function generateWebFile(
  ...
  if (!webPagePath || !webPageDir) {
    console.warn(`Could not determine Web path for ${configScreenName}.`);
    return null;
  }
...
async function renameScreenFiles(oldScreen, newScreen) {
  const {
    featurePath: oldFeaturePath,
    expoScreenDir: oldExpoScreenDir,
    webPageDir: oldWebPageDir,
  } = getScreenPaths(oldScreen.name, oldScreen.parent);
  const {
    featurePath: newFeaturePath,
    expoScreenDir: newExpoScreenDir,
    webPageDir: newWebPageDir,
  ...
  if (
    oldFeaturePath &&
    newFeaturePath &&
    (await fs.pathExists(oldFeaturePath))
  ) {
  ...
  if (
    oldExpoScreenDir &&
    newExpoScreenDir &&
    (await fs.pathExists(oldExpoScreenDir))
  ) {
  ...
  if (oldWebPageDir && newWebPageDir && (await fs.pathExists(oldWebPageDir))) {
    ...
    console.log(
      `Handled Web dir rename/move: ${oldWebPageDir} -> ${newWebPageDir}`
    );
              ...
              if (
                ...
                if (screensToAddForThisNav.length > 0) {
                  screensToAddForThisNav.forEach((screenDetail) => {
                    // screenDetail.name is full "name/index"
      ...
      return (sf) => {
        ...
        if (actions.importsToAdd?.length > 0) {
          actions.importsToAdd.forEach((imp) => {
            // imp.screenName is the featureFolderName (e.g., (home) or settings)
            if (
              !imp.componentName ||
              !imp.screenName ||
              ...
              console.warn(
                `AST: Invalid import details: ComponentName: ${imp.componentName}, ScreenName (Feature): ${imp.screenName}`
              );
              return;
            }
...
async function processBatchOfChanges(currentScreensFromParse) {
  ...
  try {
    ...
    for (const screen of deletedScreens) {
      // screen.name is full "name/index"
      console.log(
    ...
    for (const { oldScreen, newScreen } of renamedScreens) {
      ...
      const featureP = await generateFeatureScreen(
        ...
        newScreen.title || getCleanFeatureName(newScreen.name),
        true,
        true
      );
      ...
      astModificationsBatch.importsToAdd.push({
        ...
        screenName: getRouteSegmentName(newScreen.name),
      }); // Use getRouteSegmentName for feature folder
      changesEffectivelyMade = true;
    }
      ...
      if (
        !astModificationsBatch.importsToAdd.find(
          (i) =>
            ...
            i.screenName === getRouteSegmentName(newScreen.name)
        )
      ) {
        // Use getRouteSegmentName for feature folder
        astModificationsBatch.importsToAdd.push({
    ...
    for (const screen of newScreens) {
      ...
      const componentName =
        ...
        capitalizeFirstLetter(cleanFeatureNameForComponent) + "Screen";

      const featureP = await generateFeatureScreen(
        screen.name,
        componentName,
        screen.title || cleanFeatureNameForComponent,
        false,
        true
      );
      if (featureP) allGeneratedOrModifiedFiles.add(featureP);
      ...
      astModificationsBatch.importsToAdd.push({
        ...
        screenName: getRouteSegmentName(screen.name),
      }); // Use getRouteSegmentName for feature folder
      changesEffectivelyMade = true;
    }

    if (
      changesEffectivelyMade &&
      (astModificationsBatch.screensToAdd.length > 0 ||
...
async function getExistingScreenDirectories() {
  ...
  const scanAppDirRecursive = async (
    ...
    const currentParentKey =
      parentPathSegments.length > 0
        ...
        : path.basename(MONOREPO_ROOT); // Fallback for top level
  ...
  const expoAppRootItems = (await fs.pathExists(EXPO_APP_PATH))
  ...
  for (const item of expoAppRootItems) {
    if (
      ...
      await scanAppDirRecursive(
        path.join(EXPO_APP_PATH, item.name),
        screens.expo,
        [item.name]
      );
    }
  ...
  const webAppRootItems = (await fs.pathExists(WEB_APP_PATH))
  ...
  for (const item of webAppRootItems) {
    if (
      ...
      await scanAppDirRecursive(
        path.join(WEB_APP_PATH, item.name),
        screens.web,
        [item.name]
      );
    }
          ...
          if (
            ...
            statement.importClause.namedBindings.elements.forEach((element) => {
              imports.push({
                ...
                featureFolderName: featureFolderNameFromImport,
              });
            });
          }
        }
...
async function validateProjectConsistency(
  ...
  for (const screen of declaredScreens) {
    // screen.name is "configName/index"
    if (
      !screen.name ||
      !screen.componentName ||
      !screen.parent ||
    ...
    if (!actualFeatureDirs.has(featureFolderNameForImport)) {
      proposedFixes.push({
        description: `Screen '${screen.name}': Missing feature directory for '${featureFolderNameForImport}'.`,
        action: () =>
    ...
    if (!expoParentScreens || !expoParentScreens.has(screen.name)) {
      proposedFixes.push({
        description: `Screen '${screen.name}' in Expo parent '${screen.parent.name}': Missing Expo file.`,
        action: () =>
    ...
    if (!webParentScreens || !webParentScreens.has(screen.name)) {
      proposedFixes.push({
        description: `Screen '${screen.name}' in Web parent '${screen.parent.name}': Missing Web file.`,
        action: () =>
    ...
    const hasCorrectImport = actualImports.some(
      (imp) =>
        ...
        imp.featureFolderName === featureFolderNameForImport
    );
    if (!hasCorrectImport) {
      // For add_import, screenData.screenName should be the featureFolderName for the path #features/[screenName]/screen
      proposedFixes.push({
        description: `Screen '${screen.name}': Missing import for '${screen.componentName}' from feature '${featureFolderNameForImport}'.`,
        type: "ast",
        fixType: "add_import",
        screenData: {
          componentName: screen.componentName,
          screenName: featureFolderNameForImport,
        },
      });
    }
  }
  ...
  actualFeatureDirs.forEach((featureDirName) => {
    if (
      !declaredScreens.some(
        (s) => getRouteSegmentName(s.name) === featureDirName
      )
    ) {
      const dummyConfigScreenName = `${featureDirName}/index`;
      proposedFixes.push({
        description: `Orphaned feature: '${featureDirName}'.`,
        action: () =>
          deleteScreenFiles(dummyConfigScreenName, {
            name: "unknown_feature_parent",
            type: "unknown",
          }),
        type: "file",
  ...
  actualExpoScreensByParent.forEach(
    (screenNameSet, parentRouteSegmentNameFromFile) => {
      screenNameSet.forEach((configScreenNameFromFile) => {
        ...
        const declaredParent = declaredScreens.find(
          (s) =>
            s.type !== "screen" &&
            getRouteSegmentName(s.name) === parentRouteSegmentNameFromFile
        );
        const parentConfigNameToMatch = declaredParent
          ? declaredParent.name
          : parentRouteSegmentNameFromFile; // Fallback if direct parent not in config (e.g. 'Root')

        if (
          !declaredScreens.some(
            (s) =>
              s.name === configScreenNameFromFile &&
              s.parent.name === parentConfigNameToMatch
          )
        ) {
          proposedFixes.push({
            description: `Orphaned Expo screen: '${configScreenNameFromFile}' in parent group '${parentRouteSegmentNameFromFile}'.`,
            action: () =>
              deleteScreenFiles(configScreenNameFromFile, {
                name: parentConfigNameToMatch,
  ...
  actualWebScreensByParent.forEach(
    (screenNameSet, parentRouteSegmentNameFromFile) => {
      screenNameSet.forEach((configScreenNameFromFile) => {
        const declaredParent = declaredScreens.find(
          (s) =>
        ...
        const parentConfigNameToMatch = declaredParent
          ? declaredParent.name
          : parentRouteSegmentNameFromFile;
        ...
        if (
          ...
          proposedFixes.push({
            description: `Orphaned Web page: '${configScreenNameFromFile}' in parent group '${parentRouteSegmentNameFromFile}'.`,
            action: () =>
              deleteScreenFiles(configScreenNameFromFile, {
                name: parentConfigNameToMatch,
  ...
  actualImports.forEach((imp) => {
    if (
      !declaredScreens.some(
        (s) =>
          s.componentName === imp.componentName &&
          getRouteSegmentName(s.name) === imp.featureFolderName
      )
    ) {
      proposedFixes.push({
        description: `Orphaned import: '${imp.componentName}' from '#features/${imp.featureFolderName}/screen'.`,
        type: "ast",
...
async function onConfigFileChanged(changedPath) {
  ...
  if (hasPendingCliCommands) {
    ...
    for (const cmd of commandsToExecute.add || []) {
      ...
      const componentName =
        cmd.componentName || capitalizeFirstLetter(cleanFeatureName) + "Screen";
      ...
      actionsForAst.importsToAdd.push({
        componentName,
        screenName: getRouteSegmentName(cmd.name),
      }); // Use getRouteSegmentName for feature folder
    }
...
async function main() {
  ...
  if (command === "add" || command === "delete") {
    ...
    if (screenArgs.length === 0) {
      console.error(
        `Please provide at least one screen name for the '${command}' command.`
      );
      process.exit(1);
    }
...
async function handleDirectCliCommands(command, configScreenNames) {
  console.log(
    `Executing direct CLI command: ${command} for screen(s): ${configScreenNames.join(", ")}`
  );
  ...
  try {
    ...
    const astModificationsForLayout = {
      screensToAdd: [],
      screenNamesToDelete: [],
      importsToAdd: [],
      importsToRemove: [],
    ...
    if (command === "add") {
      ...
      const { parentNavigatorChoice } = await inquirer.default.prompt([
        {
          ...
          choices: [
            {
              name: "Tabs Navigator (as new tabs)",
              value: { name: "(tabs)", type: "tabs" },
            },
            {
              name: "Drawer Navigator (as new drawer items)",
      ...
      for (const configScreenName of configScreenNames) {
        ...
        if (!useDefaultsForAll) {
          ...
          if (!confirmDefaultCurrent) {
            const answers = await inquirer.default.prompt([
              {
                type: "input",
                name: "routeSegment",
                message: `Route segment for '${configScreenName}':`,
                default: routeSegmentName,
                validate: (input) =>
                  /^[a-z0-9_()]+$/.test(input) || "Invalid segment name.",
              },
              {
                type: "input",
                name: "componentName",
              ...
              },
              {
                type: "input",
                name: "href",
                message: "Screen href (full path e.g. /drawer/settings):",
                default: href,
              },
            ]);
            ...
            astModificationsForLayout.screensToAdd.push({
              name: finalConfigScreenName,
              componentName,
              title,
              icon,
              label,
              href,
              parentName: parentNameFromPrompt,
              parentType: parentTypeFromPrompt,
            });
            astModificationsForLayout.importsToAdd.push({
              componentName,
              screenName: currentFeatureFolderName,
            });
            fileGenerationDetails.push({
              configScreenName: finalConfigScreenName,
              componentName,
              parent: parentNavigatorChoice,
              title,
            });
          } else {
            // Use defaults for this specific screen
            astModificationsForLayout.screensToAdd.push({
      ...
      if (astModificationsForLayout.screensToAdd.length > 0)
        ...
        const { parent } = await inquirer.default.prompt([
          {
            type: "list",
            name: "parent",
            message: `Delete these screens from which navigator?`,
            choices: [
              {
                name: "Drawer (parent is (drawer))",
                value: { name: "(drawer)", type: "drawer" },
              },
              {
                name: "Tabs (parent is (tabs))",
                value: { name: "(tabs)", type: "tabs" },
              },
            ],
          },
      ...
      for (const configScreenName of configScreenNames) {
          ...
          const { parent } = await inquirer.default.prompt([
            {
              type: "list",
              name: "parent",
              message: `For screen '${configScreenName}', which navigator should it be deleted from?`,
              choices: [
                {
                  name: "Drawer (parent is (drawer))",
                  value: { name: "(drawer)", type: "drawer" },
        ...
        const screenToDelete = currentParsedConfig.screens.find(
          (s) =>
            s.name === configScreenName &&
            s.parent.name === parentNameFromPrompt
        );
        if (!screenToDelete) {
          console.warn(
            `Screen '${configScreenName}' not found in parent '${parentNameFromPrompt}'. Skipping AST delete.`
          );
          continue;
        }
        if (configScreenNames.length === 1) {
          const { confirmDelete } = await inquirer.default.prompt([
            {
              type: "confirm",
              name: "confirmDelete",
              message: `Confirm removal of screen '${configScreenName}' from '${parentNameFromPrompt}' in layout.tsx?`,
              default: true,
            },
          ]);
          if (!confirmDelete) {
            console.log(`Skipped AST removal of '${configScreenName}'.`);
            continue;
          }
        }
        astModificationsForLayout.screenNamesToDelete.push({
          name: configScreenName,
          parentName: parentNameFromPrompt,
        });
        if (screenToDelete.componentName)
          astModificationsForLayout.importsToRemove.push({
            componentName: screenToDelete.componentName,
          });
        astShouldChange = true;
      }
      if (
        configScreenNames.length > 1 &&
        astModificationsForLayout.screenNamesToDelete.length > 0
      ) {
    ...
    if (astShouldChange) {
      ...
      if (command === "add") {
        ...
        for (const screenToDelete of astModificationsForLayout.screenNamesToDelete) {
          const parentObject = currentParsedConfig.screens.find(
            (s) => s.name === screenToDelete.parentName && s.type !== "screen"
          ) ||
            currentParsedConfig.screens.find(
              (s) =>
                getRouteSegmentName(s.name) ===
                  getRouteSegmentName(screenToDelete.parentName) &&
                s.type !== "screen"
            ) || { name: screenToDelete.parentName, type: "unknown" };
          const deletedPaths = await deleteScreenFiles(
            screenToDelete.name,
            parentObject
          );
      ...
      if (newConfig && newConfig.screens) {
        ...
        console.log(
          "Snapshot `lastAcknowledgedConfigState` updated after CLI operations."
        );
      }
    } else {
